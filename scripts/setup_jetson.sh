#!/bin/bash
# =============================================================
# V2C Project — Jetson Orin Nano 一键部署脚本
...
检查Jetson设备() {
    ...
    if [ -f "/etc/nv_tegra_release" ]; then
        Jetson型号=$(cat /proc/device-tree/model 2>/dev/null || echo "未知 Jetson 设备")
        打印信息 "检测到 Jetson 设备：$Jetson型号"
        JetPack版本=$(cat /etc/nv_tegra_release 2>/dev/null | head -1 || echo "未知")
        打印信息 "JetPack 版本信息：$JetPack版本"
    else
        打印警告 "未检测到 Jetson 设备标识，请确认当前设备是否为 Jetson"
        打印警告 "如确认在 Jetson 上运行，可忽略此警告继续执行"
    fi
    打印信息 "当前用户：$当前用户"
    打印信息 "项目目录：$项目目录"
    打印信息 "系统版本：$(lsb_release -d 2>/dev/null | cut -f2 || uname -a)"
}
...
安装基础工具() {
    打印标题 "第二步：安装基础工具"
    打印信息 "正在安装 git、curl、wget、vim、net-tools..."
    sudo apt install -y git curl wget vim net-tools -qq
    打印信息 "基础工具安装完成 ✓"
}
...
# -------------------------------------------------------
# 第四步：安装 Python 环境与 Jetson GPIO 库
# -------------------------------------------------------
安装Python环境() {
    打印标题 "第四步：安装 Python 3 运行环境与 Jetson GPIO 库"

    # JetPack 6.1 已预装 Python 3.10，确认 pip 可用
    打印信息 "正在安装 pip 和虚拟环境工具..."
    sudo apt install -y python3-pip python3-venv -qq
    打印信息 "Python 基础环境 ✓"
    打印信息 "  Python 版本：$(python3 --version)"

    # 安装 Jetson GPIO 库（替代树莓派的 RPi.GPIO）
    打印信息 "正在安装 Jetson.GPIO 库..."
    sudo pip3 install Jetson.GPIO -q
    打印信息 "Jetson.GPIO 库安装完成 ✓"

    # 将用户加入 gpio 用户组（避免 GPIO 操作需要 sudo）
    if ! groups "$当前用户" | grep -q "gpio"; then
        打印信息 "正在将用户 $当前用户 加入 gpio 用户组..."
        sudo usermod -aG gpio "$当前用户"
        打印警告 "用户组变更需重新登录 SSH 后才能生效"
    else
        打印信息 "用户 $当前用户 已在 gpio 用户组 ✓"
    fi
}
...
配置系统服务() {
    ...
    # 将服务文件中的用户占位符替换为当前用户名和主目录
    打印信息 "正在将服务文件中的用户名替换为：$当前用户"
    sudo sed "s/User=pi/User=$当前用户/g; s/Group=pi/Group=$当前用户/g; s|/home/pi|$HOME|g" \
...
打印完成提示() {
    ...
    echo ""
    打印信息 "V2C 后端服务已成功部署到 Jetson Orin Nano！"
    echo ""
    ...
    echo ""
    打印警告 "如启用了 GPIO 功能，请重新登录 SSH 使 gpio 用户组权限生效。"
    echo ""
}
...
主流程() {
    echo ""
    echo -e "\033[36m====================================================\033[0m"
    echo -e "\033[36m    V2C Project — Jetson Orin Nano 一键部署脚本 v1.0\033[0m"
